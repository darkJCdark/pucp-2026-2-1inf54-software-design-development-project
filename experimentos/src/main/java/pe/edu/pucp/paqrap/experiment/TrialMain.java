...
public final class TrialMain {
    public static void main(String[] args)throws Exception{
        ...
        put(row,"run_id",runId,"instance_id",spec.id(),"family",spec.family(),"source",spec.source(),"instance_seed",spec.instanceSeed(),"search_seed",seed,"repetition",job.getProperty("repetition"),"algorithm",alg,"mode",cfg.text("mode","TIME"),"budget_ms",cfg.effectiveBudget(requestedBudget),"config_sha256",Json.sha256(Json.encode(cfg.asMap())),"input_sha256",job.getProperty("expected_input_sha256"));
        try{
            ...
            String status=audit.fullFeasible()?"OK":
                    ...
                    output.plan().routes().isEmpty()?"NO_SOLUTION":audit.routeValid()?"PARTIAL":"INVALID";
            String finalState=audit.fullFeasible()?"COMPLETE":!audit.routeValid()?"ERROR":
                    !output.plan().routes().isEmpty()?"INCOMPLETE":control.timedOut()?"TIMEOUT":"NO_SOLUTION";
            put(row,"status",status,"final_state",finalState,"budget_exhausted",control.timedOut(),"reheats",control.reheats(),"termination",control.timedOut()?"TIME_LIMIT":output.termination(),"initialization",output.initialization(),
                    ...
                    "orders_provably_unservable",audit.provablyUnservable(),"unruled_out_orders_fully_served",audit.servableComplete(),
                    "coverage_unruled_out_pct",Csv.number(p.orders().size()==audit.provablyUnservable()?100:100.0*audit.servableComplete()/(p.orders().size()-audit.provablyUnservable())),
                    "full_unruled_out_feasible",audit.fullServableFeasible(),"algorithm_version",solver.version(),
            ...
            e.printStackTrace();put(row,"status","ERROR","final_state","ERROR","full_feasible",false,"detail",e.getClass().getSimpleName()+": "+e.getMessage());
        }
        Properties result=new Properties();result.putAll(row);
        AtomicFiles.properties(Path.of(prefix+".result.properties"),result,"One measured trial; inspect log on ERROR");
