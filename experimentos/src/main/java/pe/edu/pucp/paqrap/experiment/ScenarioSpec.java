...
public record ScenarioSpec(String id,String family,String source,int orders,long instanceSeed,
    ...
    public ScenarioSpec {
        ...
        if(!java.util.Set.of("NORMAL","BLOCKED","REDUCED","SPLIT","REAL").contains(family))throw new IllegalArgumentException("Unknown scenario family: "+family);
        if(family.equals("REAL") != source.equals("REAL"))throw new IllegalArgumentException("REAL family and source must agree");
        if(orders<0||fromHour<0||toHour>24||toHour<=fromHour) throw new IllegalArgumentException("Invalid instance interval/count");
    }
