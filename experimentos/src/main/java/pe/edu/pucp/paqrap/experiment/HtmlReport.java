...
public final class HtmlReport {
    ...
    public static void write(Path out,List<Map<String,String>> rows)throws IOException{
        ...
        h.append("<div class='note'><b>Interpretación:</b> “OK” significa que el evaluador común valida un plan que cubre TODA la demanda. Un plan parcial barato no es un éxito. “No se encontró” no demuestra que el problema sea imposible. “Descartados por cota” usa una cota optimista de viaje Manhattan sin bloqueos, refrigerio, inventario ni otras entregas. No superar esa cota prueba imposibilidad individual en esta instancia; superarla NO prueba factibilidad. El indicador secundario de cobertura solo excluye los pedidos descartados por esa cota. Se comparan decisiones de planificación estática, no una simulación online de cinco días.</div><div class='cards'>");
        for(String alg:List.of("GRASP","SA")){
            ...
            long okServable=rr.stream().filter(r->"true".equals(r.get("full_unruled_out_feasible"))).count();
