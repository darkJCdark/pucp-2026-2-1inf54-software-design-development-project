...
public final class ExperimentMain {
    public static void main(String[] args)throws Exception{
        ...
        // Everything above is read-only: rejected resumes cannot corrupt the existing campaign.
        ...
        for(int si=0;si<specs.size();si++){
            ScenarioSpec spec=specs.get(si);ProblemInstance instance=materialized.get(spec.id());
            ...
            for(long budget:config.budgets())for(int rep=0;rep<config.seeds().size();rep++){
                ...
                for(String algorithm:order){
                    ...
                    if(!Files.exists(resultFile)){
                        ...
                        AtomicFiles.properties(resultFile,failureResult,"Failure retained in the experiment, not retried by resume");
                    }
    ...
    private static String hashDirectory(Path directory)throws IOException{
        ...
        try(var files=Files.walk(directory)){
            for(Path f:files.filter(Files::isRegularFile).sorted().toList())manifest.append(directory.relativize(f)).append(':').append(Json.sha256(Files.readAllBytes(f))).append('\n');
        }
        return Json.sha256(manifest.toString());
    }
