...
public record AnnealingConfig(double initialTemperature, double minimumTemperature,
    ...
    public AnnealingConfig {
        if (!Double.isFinite(initialTemperature) || !Double.isFinite(minimumTemperature) || initialTemperature <= 0 || minimumTemperature <= 0 || minimumTemperature >= initialTemperature) {
            throw new IllegalArgumentException("Temperatures must be positive and Tmin < T0");
        }
        if (!Double.isFinite(coolingFactor) || coolingFactor < 0.80 || coolingFactor > 0.99) {
