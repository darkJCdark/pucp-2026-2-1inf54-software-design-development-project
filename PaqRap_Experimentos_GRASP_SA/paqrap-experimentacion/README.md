...
El POM está preparado para JDK 21 y JUnit 5.11.4. La primera construcción Maven necesita descargar plugins/dependencias. **En el entorno de elaboración se verificó con `javac`, `jar` y el lanzador JUnit, no con el comando Maven.** Los scripts PowerShell se entregan para Windows, pero su ejecución no se probó en Windows aquí.
...
- El GRASP original no construye entregas divididas para un pedido mayor que la capacidad máxima individual. Se mide y se expone; no se corrigió escondidamente.
- SA depende de un constructor inicial que puede fallar aunque exista otra solución. Ese caso se registra como `NO_INITIAL_PLAN`.
...
**Estado verificado al entregar:** compilación Java; 16 pruebas JUnit sin fallos (incluyen 19 comprobaciones experimentales); 16 corridas smoke; 8 corridas de repetibilidad en dos ejecuciones; reanudación sin duplicados. Piloto y campaña formal están configurados y sus instancias cargan, pero no fueron ejecutados como campañas de algoritmos.
